...
function GameServer() {
    ...
    this.nodesPowers = []; // Food nodes
    this.nodesVirusShooter = []; // Food nodes
    this.nodesToxic = []; // Food nodes
    ...
    this.config = {
        ...
        serverMaxConnections: 500, // Maximum number of connectisons to the server. (0 for no limit)
...
GameServer.prototype.shootToxicVirus = function (client) {
	...
	var pos = client.centerPos.clone();
	
	var angle = d.angle();

    // Create cell and add it to node list
    var newCell = new Entity.VirusShooter(this, null, d, 100, angle);
	//newCell.setBoost(this.config.virusVelocity, angle);
...
GameServer.prototype.spawnCells = function () {
	...
	/*for (var i = 0; i < 2; i++) {
		if (this.nodesCoin.length < 2) {
			var coin = new Entity.Coin(this, null, this.randomPos(), 150);
			this.addNode(coin);
		}
	};*/
	
	for (var i = 0; i < 2; i++) {
		if (this.nodesPowers.length < 2) {
			var power = new Entity.PowerBoost(this, null, this.randomPos(), 150);
			this.addNode(power);
		}
	};
	
	for (var i = 0; i < 2; i++) {
		if (this.nodesToxic.length < 2) {
			var toxic = new Entity.Toxic(this, null, this.randomPos(), 100);
			toxic.color = {
				r: 0,
				g: 255,
				b: 0
